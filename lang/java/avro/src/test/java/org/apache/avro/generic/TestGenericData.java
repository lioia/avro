...
public class TestGenericData {
  ...
  public static class TestParameterized {
    ...
    public TestParameterized(Schema schema, Object datum, ExpectedResult<Boolean> expected) {
      this.schema = schema;
      this.datum = datum;
  ...
  public static class TestNonParameterized {
    ...
    public TestNonParameterized() {
      this.schema = mock(Schema.class);
      when(schema.getType()).thenReturn(Schema.Type.UNION);
      when(schema.getTypes()).thenReturn(Collections.singletonList(Schema.create(Schema.Type.INT)));
      when(schema.getIndexNamed(anyString())).thenReturn(0);
      this.datum = 3.14f;
    }
